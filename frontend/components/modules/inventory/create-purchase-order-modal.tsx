...
export function CreatePurchaseOrderModal({ children }: CreatePurchaseOrderModalProps) {
    ...
    useEffect(() => {
        ...
        ProductsService.getAll({ limit: 100 }).then(response => {
            const processed = response.data.map((p: any) => ({
                ...p,
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="w-full max-w-[95vw] lg:max-w-7xl p-0 flex flex-col gap-0 rounded-[24px] border-none shadow-2xl bg-[#FAFAFA] overflow-hidden outline-none h-[90vh]">
